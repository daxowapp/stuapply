...
export async function generateMetadata({ params }: { params: Promise<{ id: string, locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${data.names_translations?.[locale] || data.name} | ${t('studyInTurkey')}`,
    description: `${t('discoverDegree', { name: data.names_translations?.[locale] || data.name })}`,
  };
}
                 ...
                 <div className="flex flex-wrap gap-2 mb-5">
                    ...
                    {uni.founded_year && (
                      <span className="inline-flex items-center px-3 py-1.5 rounded-full bg-emerald-500/10 backdrop-blur-md border border-emerald-500/20 text-emerald-200 text-sm font-medium tracking-wide">
                         ...
                         {t('established')} {uni.founded_year}
                      </span>
                    )}
                 </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-8 border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] sticky top-24 ring-1 ring-slate-900/5">
                 ...
                 <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-linear-to-br from-slate-50 to-white rounded-2xl p-5 border border-slate-100/80 transition-shadow hover:shadow-md group">
                      ...
                      <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t('programs')}</div>
                    </div>
                    ...
                    <div className="bg-linear-to-br from-slate-50 to-white rounded-2xl p-5 border border-slate-100/80 transition-shadow hover:shadow-md group">
                      ...
                      <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t('location')}</div>
                    </div>
                 </div>
                 ...
                 <div className="mb-8">
                   ...
                   <p className="text-slate-600 leading-relaxed text-[15px]">
                     {uni.description || t('defaultDescription', { name: uni.names_translations?.[locale] || uni.name, city: uni.city_names_translations?.[locale] || uni.city || 'Turkey' })}
                   </p>
                 </div>
           ...
           <div className="lg:col-span-8 mt-4 lg:mt-0">
              <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 pb-6 border-b border-slate-200/60">
                 <div>
                    <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-2">{t('academicPrograms')}</h2>
                    <p className="text-slate-500 font-medium">{t('discoverDegree', { name: uni.names_translations?.[locale] || uni.name })}</p>
                 </div>
                 <div className="mt-4 sm:mt-0 flex items-center px-4 py-2 bg-blue-50 text-blue-700 rounded-full font-bold text-sm">
                    {programs.length} {programs.length === 1 ? t('programAvailable') : t('programsAvailable')}
                 </div>
              </div>
              ...
              {programs.length === 0 ? (
                 <div className="text-center py-24 bg-white/60 backdrop-blur-sm rounded-[2rem] border border-dashed border-slate-300">
                    ...
                    <h3 className="text-2xl font-bold text-slate-900 mb-3">{t('noProgramsTitle')}</h3>
                    <p className="text-slate-500 max-w-sm mx-auto">{t('noProgramsDesc')}</p>
                 </div>
              ) : (
